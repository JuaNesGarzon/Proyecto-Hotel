...
if (isset($_POST['enviar'])) 
{
    if (
        ...
        $sql = "INSERT INTO empleados (nombre, apellido, documento, telefono, correo, contraseña, horario, cargo)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
        
        $stmt = $conexion->prepare($sql);
        $stmt->bind_param("ssssssss", $nombre, $apellido, $documento, $telefono, $correo, $password, $horario, $cargo);
...
<head>
    ...
    <title>Crear Empleado - Hotel Deja Vu</title>
    <script>
        tailwind.config = {
            theme: {
                extend: {
...
<body class="min-h-screen bg-primary font-montserrat text-white">
    ...
    <div class="w-full max-w-md mx-auto pt-16 pb-8 px-4">
        ...
        <form action="" method="post" class="bg-white/10 backdrop-blur-lg rounded-xl shadow-lg p-8">
            <h3 class="text-3xl font-playfair font-bold text-white mb-6 text-center">Agregar Empleado</h3>
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <input type="datetime-local" name="horario" id="horario" required class="w-full px-4 py-2 rounded-lg bg-white/10 border border-white/20 text-white focus:outline-none focus:ring-2 focus:ring-coral">
                </div>
                <div>
                    <label for="cargo" class="block text-white mb-1 font-medium">Cargo:</label>
                    <input type="number" name="cargo" id="cargo" required class="w-full px-4 py-2 rounded-lg bg-white/10 border border-white/20 text-white placeholder-white/50 focus:outline-none focus:ring-2 focus:ring-coral">
                </div>
            </div>
